...
void main()
{
	...
	fragmentColor = texture(texture_diffuse1, v_TextCoord);// + texture(texture_specular1, v_TextCoord) + texture(texture_normal1, v_TextCoord) + texture(texture_height1, v_TextCoord);
}
...
vec4 CalculateDirLight(DirLight light, vec3 normal, vec3 viewDir) {
	...
	vec4 ambient  = vec4(light.ambient ,  1.0)	* color		*	 texture(texture_diffuse1, v_TextCoord);
